...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // sicash
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            SICashTxConverter converter(tx, NULL);
            ...
            for(SICashTransaction sicashTransaction : sicashTransactions){
                ...
                if(sicashTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s SICASH additional fees, %d delta bytes\n",
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    SICashTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<SICashTransaction>(1, callTransaction), blockGasLimit);
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // sicash
    SICashDGP sicashDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setSICashSchedule(sicashDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = sicashDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = sicashDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = sicashDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // sicash
...
///////////////////////////////////////////////////////////////////////////////////////// sicash
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            SICashTxConverter convert(tx, &view, &block.vtx);
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertSICashTX.first, blockGasLimit);
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertSICashTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertSICashTX.first[k].from(), resultConvertSICashTX.first[k].to(),
