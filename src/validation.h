...
extern bool fAddressIndex; // sicash
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashSICashRoot,
    const dev::h256& oldHashStateRoot, const std::vector<SICashTransaction>& transactions);
...
public:
    ...
    SICashTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<SICashTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
