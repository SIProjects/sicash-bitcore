...
void Shutdown()
{
    ...
    if (pwalletMain)
    {
        StakeSICashs(false, pwalletMain);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::path stateDir = GetDataDir() / "stateSICash";
                ...
                boost::filesystem::path sicashStateDir = GetDataDir() / "stateSICash";

                bool fStatus = boost::filesystem::exists(sicashStateDir);
                ...
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::sicashMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(sicashStateDir.string());
