...
{
    CBitcoinAddress sicashAddress(strSICashAddress);
    if(sicashAddress.IsValid()){
        CKeyID keyid;
        sicashAddress.GetKeyID(keyid);
...
bool ToSICashAddress(const std::string& strHash160, std::string& strSICashAddress)
{
    ...
    CKeyID keyid(key);
    CBitcoinAddress sicashAddress;
    sicashAddress.Set(keyid);
    if(sicashAddress.IsValid()){
        strSICashAddress = sicashAddress.ToString();
        return true;
    }
    return false;
}
