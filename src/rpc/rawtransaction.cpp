...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the sicash address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
            ...
            "         \"amount\":x.xxx,                (numeric, optional) Value in SICASH to send with the call, should be a valid amount, default 0\n"
